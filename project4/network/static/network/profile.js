...
document.addEventListener('DOMContentLoaded',function(){
    document.querySelector('#user_posts').innerHTML = "<h3>Posts</h3>";
    fetch(`/view_post/${document.getElementById('user_name').value}`)
    .then(response => response.json())
    .then(posts => {
        posts.reverse().forEach((post) => {
            var date = new Date(post.fields.timestamp)
            var username = post.fields.username
            document.querySelector("#user_posts").innerHTML += `<h5><b>${username}<b></h5>`;
            document.querySelector("#user_posts").innerHTML += `${post.fields.body} <br>`;
            document.querySelector("#user_posts").innerHTML += `${date.toDateString()} at ${date.toLocaleTimeString()}<br>`;
            document.querySelector("#user_posts").innerHTML += `<p>&#10084; ${post.fields.likes}</p><hr>`
        })
    })
})
*/
    ...
    .then(posts => {
        ...
        if (array_len > 10){
            ...
            posts.reverse().forEach((post) => {
                var date = new Date(post.fields.timestamp)
                var username = post.fields.username
                document.querySelector("#user_posts").innerHTML += `<h5><a href="profile/${username}"><b>${username}<b></a></h5>`;
                current_user = document.getElementById("user_name").value
                if (username === current_user){
                    document.querySelector("#user_posts").innerHTML += `<button onclick = 'edit("${post.pk}","${post.fields.body}")'>Edit</button> <br>`;
                }
                document.querySelector("#user_posts").innerHTML += `<div id = "post_id${post.pk}">${post.fields.body} <br></div>`;
                document.querySelector("#user_posts").innerHTML += `${date.toDateString()} at ${date.toLocaleTimeString()}<br>`;
                document.querySelector("#user_posts").innerHTML += `<p id = "like${post.pk}" onclick = "liked('${post.pk}','${post.fields.likes}')">&#128077; ${post.fields.likes}</p><hr>`;
            })
        }
    })
})
...
function newcontent(id) {
    //console.log(`${document.getElementById('newBody').value}`)
...
function liked(post_pk, num_likes) {
    current_user = document.getElementById("user_name").value //change
